...
export default function Blob() {
  return (
    <section id="blob" className="relative py-28 px-4 sm:px-6 overflow-hidden">
      {/* Background Ambient Glow */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden -z-10">
        <div className="absolute top-1/2 left-1/4 w-[40rem] h-[40rem] bg-purple-600/10 rounded-full blur-[160px]" />
          ...
          {featuredArticles.map((article, index) => {
            ...
            return (
              <motion.div
                ...
                className="h-full"
              >
                <TiltCard glowColor={article.glowColor} className="p-7 sm:p-8 flex flex-col justify-between h-full group">
                  <div className="space-y-5">
                    ...
                    <div className="flex items-center justify-between">
                      ...
                      <span className="px-3 py-1 rounded-full text-xs font-mono font-medium bg-primary/10 border border-primary/20 text-primary">
                        {article.badge}
                      </span>
                    </div>
                    ...
                    <div>
                      ...
                      <p className="text-xs font-medium text-cyan-300 mt-1 font-mono">
                        {article.subtitle}
                      </p>
                    </div>
                    ...
                    {/* Achievements List */}
                    <div className="space-y-1.5 p-3.5 rounded-2xl bg-white/[0.03] border border-white/[0.06]">
                    ...
                    <div className="flex flex-wrap gap-1.5 pt-1">
                      {article.tech.slice(0, 4).map((tech) => (
                        <span
                          key={tech}
                          className="px-2.5 py-0.5 rounded-full text-[11px] font-mono bg-white/[0.04] border border-white/[0.08] text-white/80"
                        >
                          {tech}
                        </span>
                      ))}
                  ...
                  {/* Bottom Actions */}
                  <div className="flex flex-col sm:flex-row items-center gap-2.5 pt-6 mt-6 border-t border-white/[0.08]">
                    <a
                      href={article.source}
                      target="_blank"
                    ...
                    >
                      <Button
                        variant="outline"
                        size="sm"
                        className="w-full h-9 rounded-xl glass-pill hover:border-white/30 text-xs font-medium"
                      >
                    ...
                    <Link href={article.article} className="w-full sm:flex-1">
                      <Button
                        size="sm"
                        className="w-full h-9 rounded-xl bg-gradient-to-r from-primary to-purple-600 hover:from-primary/90 text-white text-xs font-medium shadow-[0_0_15px_rgba(59,130,246,0.3)] group/read"
                      >
