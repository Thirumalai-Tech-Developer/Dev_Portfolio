...
export default function Skills() {
  ...
  return (
    <section ref={skillsRef} className="relative py-16 sm:py-20 md:py-24 lg:py-28 px-3 xs:px-4 sm:px-6 lg:px-8 w-full max-w-full overflow-hidden" id="skills">
      {/* Background Glow */}
      <div className="pointer-events-none absolute inset-0 overflow-hidden -z-10">
        <div className="absolute top-1/2 left-1/3 w-72 sm:w-[36rem] h-72 sm:h-[36rem] bg-indigo-600/10 rounded-full blur-[100px] sm:blur-[140px]" />
      </div>
      ...
      <div className="max-w-7xl mx-auto space-y-10 sm:space-y-12 md:space-y-16">
          ...
          {skillHighlights.map((hl, index) => {
            ...
            return (
              <motion.div
                ...
                className="h-full"
              >
                <div className={`p-5 sm:p-6 rounded-2xl glass-card border bg-gradient-to-br ${hl.color} hover:scale-[1.02] transition-transform duration-300 h-full flex flex-col justify-between`}>
                  <div>
                    <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-xl bg-white/10 backdrop-blur-md flex items-center justify-center mb-3 sm:mb-4 border border-white/15">
                      <Icon className="w-5 h-5 sm:w-6 sm:h-6" />
                    </div>
                    <h3 className="text-lg sm:text-xl font-bold text-foreground mb-1.5 sm:mb-2">{hl.title}</h3>
                    <p className="text-xs sm:text-sm text-muted-foreground leading-relaxed">{hl.desc}</p>
                  </div>
                </div>
              </motion.div>
            );
        ...
        <motion.div
          ...
          className="rounded-2xl overflow-hidden shadow-2xl w-full max-w-full"
        >
          <TechMarquee />
        </motion.div>
        ...
        <motion.div 
          layout
          className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-3.5 sm:gap-4 md:gap-5"
        >
          {filteredSkills.map((skill, index) => {
            ...
            return (
              ...
              >
                <TiltCard glowColor={skill.glow} className="p-4 sm:p-5 flex flex-col justify-between h-full">
                  <div>
                    {/* Top row: Icon, Name & Category */}
                    <div className="flex items-center justify-between mb-3 sm:mb-4">
                      <div className="flex items-center gap-2.5 sm:gap-3 min-w-0">
                        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl bg-white/[0.06] border border-white/10 p-2 flex items-center justify-center backdrop-blur-md flex-shrink-0">
                          <Icon className="w-4 h-4 sm:w-5 sm:h-5 text-primary" />
                        </div>
                        <div className="min-w-0">
                          <h4 className="font-semibold text-xs sm:text-sm text-foreground truncate">{skill.name}</h4>
                          <span className="text-[10px] sm:text-[11px] text-muted-foreground truncate block">{skill.experience}</span>
                        </div>
                      </div>

                      <Badge variant="outline" className="text-[9px] sm:text-[10px] px-1.5 sm:px-2 py-0.5 border-white/10 bg-white/5 text-muted-foreground flex-shrink-0 ml-2">
                    ...
                    <div className="space-y-1 sm:space-y-1.5">
                      ...
                      <div className="w-full bg-white/[0.08] rounded-full h-1.5 sm:h-2 overflow-hidden p-[1px]">
                        <motion.div
